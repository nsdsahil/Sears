...
import allProducts from "../docs/allProducts";
	...
	return (
		<Box>
			<Flex justifyContent={"space-between"}>
				...
				>
					...
					>
						{currentItems.map((product) => (
							<GridItem  >
								<Box>
									<img width={"100%"}  src={product["img-fluid-product-src"]} alt="Image not found" />
								</Box>
								<Box  paddingLeft={2}>
								<Text >{product["money 2"]}</Text>
								<Text color='red'> <strike>{product["money"]}</strike></Text>
								<Text>{product["final-price-display"]}</Text>
								...
								<ReactStars
									...
									activeColor="#ffd700"
								/>
							<Button color={color.secondary} bgColor={color.primary} >Add to cart</Button></Box>
							</GridItem>
						))}
					</Grid>
				</Box>
				...
				<Flex padding={2} margin={"auto"} width={"20%"}  alignItems={"center"} justifyContent={"space-between"} >
                <Button size='sm' color={color.secondary} bgColor={color.primary} onClick={() => paginate(currentPage - 1)}>Previous</Button>
				<Text>Page {currentPage}</Text>
                <Button size='sm' color={color.secondary} bgColor={color.primary} onClick={() => paginate(currentPage + 1)}>Next</Button></Flex>
